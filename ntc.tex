...
Een NTC is een semiconductor weerstand en is erg gevoelig voor kleine variaties
...
\section{Bepalen van de weerstandswaarde van de NTC}
...
uitgangsspanning van de spanningsdelen aangeboden aan een Schmitt-trigger die op
bepaalde spanningen (en dus temperaturen) omschakelt.
...
R_{opt} = \sqrt{\dfrac{1}{n}\cdot n} = 1
\end{equation}
voor $n$ = 2, 3, 4, 10. Het voordeel hiervan is dat $R_{opt}$ nu altijd 1 is
...
We nemen als bronspanning $U_B=5$ V. Stel dat $U_0 = 2,1$ V. Dan is
...
\begin{equation}
H(T) = \dfrac{R_{}opt}{\rntc + R_{opt}}=\dfrac{R_{opt}}{A\cdot\text{e}^{\,(B/T)}+R_{opt}}
\end{equation}
...
\begin{equation}
\setlength\fboxsep{2ex}
\boxed{R_{opt} = \sqrt{R_K\cdot R_G}}
\end{equation}
